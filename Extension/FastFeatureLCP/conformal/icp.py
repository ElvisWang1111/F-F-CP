...
class _DisableDropout:
    """Temporarily replace all Dropout layers in a module with Identity()."""
    def __init__(self, root: nn.Module):
        self.root = root
        self._backup = []

    def __enter__(self):
        for name, module in list(self.root.named_modules()):
            if isinstance(module, (nn.Dropout, nn.Dropout1d, nn.Dropout2d, nn.Dropout3d)):
                ...
                attr = parts[-1]
                self._backup.append((parent, attr, getattr(parent, attr)))
                setattr(parent, attr, nn.Identity())
        return self

    def __exit__(self, exc_type, exc, tb):
        for parent, attr, original in self._backup:
...
class FeatErrorErrFunc(RegressionErrFunc):
    def __init__(self, feat_norm, kernel_weight):
    ...
    def apply_inverse_weighted(self, nc, significance, weights):
        nc = np.asarray(nc).reshape(-1)
        ...
        thr = 1.0 - significance
        j = np.searchsorted(cdf, thr, side="left")
        j = min(max(j, 0), nc_sorted.size - 1)

        q = float(nc_sorted[j])

        return np.vstack([q, q])

    def calculate_H(self, x_cal, x_test):

        distances = np.linalg.norm(x_cal[:, None, :] - x_test[None, :, :], axis=2)
        H = np.exp(-0.15 * distances)
...
class FeatRegressorNc(BaseModelNc):
    def __init__(self, model,
                 ...
                 normalizer=None, beta=1e-6, g_out_process=None, kernel_weight=0.15):
        ...
        self.kernel_weight = kernel_weight
        err_func = FeatErrorErrFunc(feat_norm=self.feat_norm, kernel_weight=self.kernel_weight)
    ...
    def coverage_loose(self, x, y, z_pred, steps, val_significance):
        ...
        for i, step_cal_score in enumerate(cal_score_list):
            val_predictions = self.predict(x=val_x.detach().cpu().numpy(), nc=step_cal_score, x_cal=cal_x,
    ...
    def predict(self, x, nc, significance=None, x_cal=None):
        n_test = x.shape[0]
        prediction = self.model.predict(x)
        ...
        if significance:
            ...
            if isinstance(x, torch.Tensor):
                x_t = x.to(self.model.device)
                x_np = x.detach().cpu().numpy()
            else:
                x_t = torch.from_numpy(x).to(self.model.device)
                x_np = x
            x_cal_np = x_cal if isinstance(x_cal, np.ndarray) else np.asarray(x_cal)

            H = self.err_func.calculate_H(x_cal_np, x_np)  # (n_test, n_cal)

            row_sums = H.sum(axis=1, keepdims=True)
            W = H / row_sums

            feat_err_dist = self.err_func.apply_inverse_weighted(nc, significance, W)
            ...
            if prediction.ndim > 1:
                ...
                self.model.model.eval()
                if hasattr(self.model.model, "g"):
                    self.model.model.g.eval()
                with torch.no_grad():
                    z_all = self.model.model.encoder(x_t).detach()  # (n_test, zdim, ...)

                out_dim = self.model.model.out_shape if hasattr(self.model.model, "out_shape") else None
                if out_dim is None:
                    with torch.no_grad():
                        y0 = self.model.model.g(z_all[:1])
                        if self.g_out_process is not None:
                            y0 = self.g_out_process(y0)
                        out_dim = int(y0.numel())

                with _DisableDropout(self.model.model.g if hasattr(self.model.model, "g") else self.model.model):
                    for i in range(n_test):
                        z_i = z_all[i:i+1] 
                        with torch.no_grad():
                            yi = self.model.model.g(z_i)
                        lirpa_model = BoundedModule(self.model.model.g, z_i.clone())
                        ptb = PerturbationLpNorm(norm=self.feat_norm, eps=feat_err_dist[0][0])
                        my_input = BoundedTensor(z_i, ptb)
                        ...
                        if self.g_out_process is not None:
                            lb = self.g_out_process(lb)
                            ub = self.g_out_process(ub)
                        lb = lb.detach().cpu().numpy().reshape(-1)
                        ub = ub.detach().cpu().numpy().reshape(-1)
...
class FFCPErrorErrFunc(RegressionErrFunc):
    def __init__(self):
        super(FFCPErrorErrFunc, self).__init__()

    def apply_grad(self, prediction, y, grad):

        err = np.abs(prediction.squeeze() - y)
        if err.ndim > 1:
            err = np.linalg.norm(err, ord=np.inf, axis=1)
        grad_norms = np.array([np.linalg.norm(g) for g in grad])
        err = np.divide(err, grad_norms, where=grad_norms!=0)

        return err

    def apply_inverse_grad(self, nc, significance, weights):

        indices = np.argsort(nc)
        sorted_nc = nc[indices]
        sorted_weights = weights[indices]
        cumulative_weights = np.cumsum(sorted_weights)
        quantile_index = np.searchsorted(cumulative_weights, 1-significance) - 1
        quantile_index = min(max(quantile_index, 0), nc.size - 1)

        return sorted_nc[quantile_index]
...
class FFCP_RegressorNc(BaseModelNc):
    def __init__(self, model, err_func=FFCPErrorErrFunc(), normalizer=None, beta=1e-6):
        super(FFCP_RegressorNc, self).__init__(model, err_func, normalizer, beta)
        err_func = FFCPErrorErrFunc()

    def predict(self, model, x_cal, y_cal, x_test, y_test, layer_index, significance=None):

        n_test = x_test.shape[0]
        prediction_cal, grads_intermediate_cal, gradx_cal = model.predict_with_grads(x_cal, layer_index)
        nc = self.err_func.apply_grad(prediction_cal, y_cal, grads_intermediate_cal)
        prediction, grads_intermediate, gradx = model.predict_with_grads(x_test, layer_index)

        weights = self.err_func.calculate_H(x_cal, x_test)
        row_sums = weights.sum(axis=1, keepdims=True)
        weights_normalized = weights / row_sums

        grad_norms = np.array([np.linalg.norm(g) for g in grads_intermediate])

        n_test = x_test.shape[0]
        intervals = np.zeros((n_test, prediction.shape[1] if prediction.ndim > 1 else 1, 2))

        for i in range(n_test):

            weighted_quantile = self.err_func.apply_inverse_grad(nc, significance, weights_normalized[i])
            grad_norm = np.linalg.norm(grads_intermediate[i]) if grads_intermediate.ndim > 1 else np.linalg.norm(
                grads_intermediate)

            if prediction.ndim > 1:
                for j in range(prediction.shape[1]):  # For each output dimension
                    intervals[i, j, 0] = prediction[i, j] - weighted_quantile * grad_norm
                    intervals[i, j, 1] = prediction[i, j] + weighted_quantile * grad_norm
            else:
                intervals[i, 0, 0] = prediction[i] - weighted_quantile * grad_norm
                intervals[i, 0, 1] = prediction[i] + weighted_quantile * grad_norm
...
class IcpRegressor(BaseIcp):
    ...
    def if_in_coverage(self, x, y, significance, nc_cal_scores):
        self.nc_function.model.model.eval()

        err_dist = self.nc_function.score(x, y)

        threshold = self.nc_function.err_func.apply_inverse(
            nc_cal_scores, significance
        )[0][0]

        return err_dist < threshold
...
class LCPErrorErrFunc(RegressionErrFunc):
    def __init__(self):
        super(LCPErrorErrFunc, self).__init__()

    def apply(self, prediction, y, grad):
...
class LCP_RegressorNc(BaseModelNc):
    def __init__(self, model, err_func=LCPErrorErrFunc(), normalizer=None, beta=1e-6):
        super(LCP_RegressorNc, self).__init__(model, err_func, normalizer, beta)
        err_func = FFCPErrorErrFunc()
    ...
    def predict(self, model, x_cal, y_cal, x_test, y_test, layer_index, significance=None):
        ...
        n_test = x_test.shape[0]
        prediction_cal, grads_intermediate_cal, gradx_cal = model.predict_with_grads(x_cal, layer_index)
        nc = self.err_func.apply(prediction_cal, y_cal, grads_intermediate_cal)
        prediction, grads_intermediate, gradx = model.predict_with_grads(x_test, layer_index)
        ...
        for i in range(n_test):
            ...
            weighted_quantile = self.err_func.apply_inverse_grad(nc, significance, weights_normalized[i])
            # Adjust the computation based on the dimensionality of the prediction
            if prediction.ndim > 1:
                for j in range(prediction.shape[1]):  # For each output dimension
                    intervals[i, j, 0] = prediction[i, j] - weighted_quantile
                    intervals[i, j, 1] = prediction[i, j] + weighted_quantile
            else:
                intervals[i, 0, 0] = prediction[i] - weighted_quantile
                intervals[i, 0, 1] = prediction[i] + weighted_quantile
