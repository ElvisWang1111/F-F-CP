...
def lirpa_softmax_bounds(z: torch.Tensor, eps: float,
                         method="CROWN-Optimized", ob_iter=20):

    model = SoftmaxWrapper().eval() 
    lirpa_model = BoundedModule(model, z)
    ptb = PerturbationLpNorm(norm=float("inf"), eps=eps)
    z_perturbed = BoundedTensor(z, ptb)

    if "Optimized" in method:
        lirpa_model.set_bound_opts({
            "optimize_bound_args": {"ob_iteration": ob_iter,
                                    "ob_lr": 0.1,
                                    "ob_verbose": 0}
        })

    lb, ub = lirpa_model.compute_bounds(x=(z_perturbed,), method="IBP")
...
class ConformalModel_fraps(nn.Module):
    def __init__(self, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False,
                 ...
                 LAC=False, calib_method='IBP', calib_bs=32, cmethod='backward'):
        super(ConformalModel_fraps, self).__init__()
        self.model = model 
        self.alpha = alpha
    ...
    def forward(self, *args, randomized=None, allow_zero_sets=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            eps_i = 0.1
            lb, scores = lirpa_softmax_bounds(logits, eps_i, method="CROWN-Optimized", ob_iter=20)
