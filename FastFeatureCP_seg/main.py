...
def main(train_loader, cal_loader, test_loader, args):
    ...
    coverage_fcp, length_fcp = [], []
    print("image_shape:",image_shape)
    print("out_shape:",args.height*args.width*num_classes)
    ...
    for x_test, discrete_y_test, y_test in tqdm(test_loader):
        ...
        if args.visualize:
            ...
            for img_interval in show_interval:
                ...
                visualize(img_interval, height=args.height, width=args.width, save_dir=os.path.join(f'visualization/FCP_seed{seed}', img_name))
                img_idx += 1

    print("FFCP starts running")
    FFCP_nc = FFCP_RegressorNc(mean_estimator)
    for layer_index in range(1):
        layer_index = 0
        FFCP_nc.calibrate_batch(cal_loader, mean_estimator, layer_index)
        FFCP_test_intervals = []
        FFCP_all_y_test = []
        img_idx = 0
        for x_test, _, y_test in tqdm(test_loader):
            intervals_2 = FFCP_nc.predict(mean_estimator, x_test, y_test, layer_index, significance=alpha)
            FFCP_test_intervals.append(intervals_2)
            FFCP_all_y_test.append(y_test.cpu().numpy())
            if args.visualize:
                loglog_interval = np.exp(-np.exp(intervals_2))
                show_interval = np.abs(loglog_interval[..., 1] - loglog_interval[..., 0])
                for img_interval in show_interval:
                    img_interval = img_interval.reshape(19, args.height, args.width).mean(axis=0)
                    img_name = os.path.basename(
                        test_loader.dataset.dataset.train_data[test_loader.dataset.indices[img_idx]])
                    visualize(img_interval, height=args.height, width=args.width,
                              save_dir=os.path.join(f'visualization/FFCP_seed{seed}', img_name))
                    img_idx += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset-dir", type=str, default="/root/autodl-tmp",
    ...
    parser.add_argument("--feat_lr", "--fl", type=float, default=1e-1)
    parser.add_argument("--feat_step", "--fs", type=int, default=10)
    parser.add_argument("--feat_norm", "--fn", default=-1)
    parser.add_argument("--cert_method", "--cm", type=int, default=0, choices=[0, 1, 2, 3])

    parser.add_argument("--visualize", action="store_true", default=True, help="visualize the length in the image")
    ...
    for seed in args.seed:
        ...
        run = main(train_loader, cal_loader, test_loader, args)
