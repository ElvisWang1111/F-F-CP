...
class FCNHead(nn.Sequential):
    ...
    def forward(self, inputs, layer_index = 0, return_intermediate = False):

        x = inputs.view(inputs.shape[0], *self.feature_shape)
    ...
    def predict_with_grads(self, X, layer_index, y_index):
        self.eval()
        ...
        with torch.enable_grad():  # Enable gradient computation
            ...
            output = outputs[:, y_index,:,:]  # Select each output separately
            output.backward(torch.ones_like(output), retain_graph=True)  # Perform backward pass

            # Get the gradients
            intermediate_grads = intermediate.grad.cpu()  # Gradients of output w.r.t. intermediate
            outputs_grads = X.grad.cpu()  # Gradients of output w.r.t. X

            # Reset gradients
            intermediate.grad.zero_()
            X.grad.zero_()

        return outputs.detach().cpu().numpy(), intermediate_grads.detach().cpu().numpy(), outputs_grads.detach().numpy()
...
class FCN(nn.Module):
    ...
    def output_post_process1(self, output):
        output = F.interpolate(output, size=self.img_shape, mode='bilinear', align_corners=False) # (512, 1024)
        # remove the first dimension "unlabeled"
        output = output[:, 1:, ...]
        output = output.view(output.shape[0], -1)
        return output
    ...
    def predict_with_grads(self, x, index_layer=0, y_layer = 0):

        self.eval()
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        x = x.to(device)
        output1 = self.encoder.forward(x)
        output2, intermediate_grads2, outputs_grads2 = self.g.predict_with_grads(output1, index_layer, y_layer)

        return output2, intermediate_grads2, outputs_grads2
