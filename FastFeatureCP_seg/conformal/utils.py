...
def compute_overall_coverage(y_test, y_lower, y_upper, alpha, method_name):

    in_interval = (y_test >= y_lower) & (y_test <= y_upper)
    
    coverage_per_sample = np.sum(in_interval, axis=1)
    overall_coverage = y_test.shape[1]
    coverage_ratio = coverage_per_sample / overall_coverage
    coverage_ratio = np.mean(coverage_ratio)
    
    max_length = np.mean(y_upper - y_lower)
    
    return coverage_ratio, max_length
