...
def _replace_dropout_with_identity(module: nn.Module) -> None:
...
class FFCPErrorErrFunc(RegressionErrFunc):
    ...
    def apply_inverse_grad(self, nc, significance):
        nc = nc.reshape(-1)
        sorted_data = np.sort(nc, axis=0)  # (10, 2490368)

        significance = 1 - significance
        border = int(np.floor(significance * (nc.shape[0] + 1))) - 1
        border = min(max(border, 0), nc.shape[0] - 1)

        quantiles = sorted_data[border]  
        quantile_matrix = np.vstack([quantiles, quantiles])
        
        return quantile_matrix


class FFCP_RegressorNc(BaseModelNc):
    def __init__(self, model, err_func=FFCPErrorErrFunc(), normalizer=None, beta=1e-6, condition = None):
    ...
    def calibrate_batch(self,dataloader, model, layer_index):
        if self.conditional:
            raise NotImplementedError

        else:
            self.categories = np.array([0])
            cal_scores = self.score_batch(dataloader,layer_index)
            self.cal_scores1 = cal_scores
            self.err_dist = self.err_func.apply_inverse_grad(self.cal_scores1, 0.1)  # ABS

    def score_batch(self, dataloader, layer_index):
        ret_val = []
        for x, _, y in tqdm(dataloader):
            prediction = self.model.predict(x)
            grad_list = []
            for i in range(20):  # Iterate over each output
                _, grad, _ = self.model.model.predict_with_grads(x, layer_index, i)

                grad = torch.tensor(grad)
                norm_tensor = torch.norm(grad, p=2, dim=1, keepdim=True)
                grad_list.append(norm_tensor)

            norm_concatenated = torch.cat(grad_list, dim=1)
            grad = self.model.model.output_post_process1(norm_concatenated)
            batch_ret_val = self.err_func.apply_grad(prediction, y.detach().cpu().numpy(), grad)
            ret_val.append(batch_ret_val)
    ...
    def predict(self, model, x_test, y_test, layer_index, significance=None):
        ...
        grad_list = []
        for i in range(20):  # Iterate over each output
            prediction, grads_intermediate, gradx = self.model.model.predict_with_grads(x_test, layer_index, i)
            grads_intermediate = torch.from_numpy(grads_intermediate)
            norm_tensor = torch.norm(grads_intermediate, p=2, dim=1, keepdim=True)
            grad_list.append(norm_tensor)

        norm_concatenated = torch.cat(grad_list, dim=1)
        prediction_no_grad = self.model.predict(x_test)

        grads_intermediate = self.model.model.output_post_process1(norm_concatenated)

        grads_intermediate = grads_intermediate.cpu().numpy()

        grad = grads_intermediate

        grad_norms = grad  

        if significance :

            intervals = np.zeros((x_test.shape[0], model.model.out_shape, 2))
            err_dist = self.err_dist
            
            intervals[..., 0] = prediction_no_grad - err_dist[0, :] * grad_norms
            intervals[..., 1] = prediction_no_grad + err_dist[1, :] * grad_norms
...
class FeatRegressorNc(BaseModelNc):
    ...
    def predict(self, x, nc, significance=None):
        ...
        if significance:
            ...
            with torch.no_grad():
                z = self.model.model.encoder(x)

            device = z.device
            dtype = z.dtype
            g = self.model.model.g.to(device=device, dtype=dtype)
            g.eval()
            _replace_dropout_with_identity(g)
            dummy = torch.empty_like(z, device=device, dtype=dtype)
            lirpa_model = BoundedModule(g, dummy)
            lirpa_model.eval()
            eps = feat_err_dist[0][0]
            eps = torch.as_tensor(eps, device=device, dtype=dtype)
            ptb = PerturbationLpNorm(norm=self.feat_norm, eps=eps)
